...
async function uploadMainProduct(page, projectId) {
  ...
  await page.getByTestId('canvas-asset-uploader').locator('input[type="file"]').setInputFiles({
...
test('persists async progress through refresh and cancels the saved provider task exactly once', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('canvas-generation-status')).toContainText(generation.id);
  ...
  await expect(page.getByTestId('canvas-generation-status')).toContainText('cancelled');
  const audit = await (await request.get('/_e2e/runtime-audit')).json();
...
test('partially failed item retries once and browser reload pages immutable result versions', async ({ page, request }) => {
  ...
  const selector = page.getByRole('combobox', { name: '选择结果版本' });
  await expect(selector).toBeVisible();
  await expect(selector.locator('option')).toHaveCount(2);
  const selectableVersionIds = await selector.locator('option').evaluateAll((options) => (
  ...
  await selector.selectOption(selectableVersionIds[0]);
  await waitForSaved(page);
  await page.reload({ waitUntil: 'domcontentloaded' });
  await expect(selector).toHaveValue(selectableVersionIds[0]);
});
...
test('unknown submission stays actionable until explicit retry and succeeds without external network', async ({ page, request }) => {
  ...
  await page.reload({ waitUntil: 'domcontentloaded' });
  await expect(page.getByTestId('canvas-generation-status')).toContainText('unknown');
...
test('capacity rejection is safe before generation and the retained intent succeeds after capacity recovers', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('canvas-generation-status')).toContainText('insufficient storage capacity');
...
test('double click plus a committed response drop reuses the pending idempotency key after refresh', async ({ page, request }) => {
  ...
  await Promise.all([
    page.getByTestId('canvas-generate').click(),
    page.getByTestId('canvas-generate').click(),
  ]);
  ...
  await page.reload({ waitUntil: 'domcontentloaded' });
  await expect(page.getByTestId('canvas-generation-status')).toContainText(committed.id);
...
test('saved generated result exports only after explicit board mode and format choices', async ({ page, request }) => {
  ...
  await page.reload({ waitUntil: 'domcontentloaded' });
  await waitForSaved(page);
  const resultSelector = page.getByRole('combobox', { name: '选择结果版本' });
  await expect(resultSelector.locator('option')).toHaveCount(2);
  const composedAssetId = await resultSelector.locator('option').nth(1).getAttribute('value');
  expect(composedAssetId).toBeTruthy();
  await resultSelector.selectOption(composedAssetId);
  await waitForSaved(page);
