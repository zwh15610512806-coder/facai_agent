...
class DurableTask(Base):
    ...
    queue_group = Column(String(30), nullable=False, default="maintenance", index=True)
    job_run_id = Column(Integer, ForeignKey("job_runs.id", ondelete="SET NULL"), index=True)
    ...
    lease_owner = Column(String(100), index=True)
...
class JobRun(Base):
    ...
    queue_group = Column(String(30), nullable=False, default="maintenance", index=True)
    owner_key = Column(String(64), index=True)
    origin_path = Column(String(500))
    source_ref = Column(String(200))
    idempotency_key = Column(String(128), index=True)
    parent_job_id = Column(Integer, ForeignKey("job_runs.id", ondelete="SET NULL"), index=True)
    status = Column(String(20), nullable=False, default="pending", index=True)
    ...
    partial_result = Column(JSON, default=dict)
