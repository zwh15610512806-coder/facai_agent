...
- All application pages and business APIs are usable without a password, role token, or login cookie.
...
- Add failing tests proving general pages/APIs and the integration center work without credentials.
- Add purge contract tests proving the confirmation phrase is required and passwords are neither accepted nor checked.
- Remove or rewrite tests whose sole purpose is password hashing, login sessions, or role tokens.
- Search production sources and shipped assets for obsolete password-login strings and environment variables.
- Run targeted authentication, integration, and purge tests during implementation.
...
- **Risk:** old frontend assets continue prompting for tokens.  
  **Mitigation:** rebuild affected assets and test the shipped bundle, not only source modules.
