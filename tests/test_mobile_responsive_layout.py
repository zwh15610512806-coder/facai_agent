...
class SharedMobileResponsiveCssTests(unittest.TestCase):
    ...
    def test_shared_tools_launcher_is_bottom_right_and_mobile_uses_nav(self):
        launcher = re.search(r"\.facai-tools-launcher\s*\{(?P<body>.*?)\n\}", self.css, flags=re.S)
        toggle = re.search(r"\.facai-tools-toggle\s*\{(?P<body>.*?)\n\}", self.css, flags=re.S)
        mobile = re.search(r"@media \(max-width: 768px\)\s*\{(?P<body>.*?)\n\}", self.css, flags=re.S)

        self.assertIsNotNone(launcher)
        self.assertIsNotNone(toggle)
        self.assertIsNotNone(mobile)
        launcher_body = launcher.group("body")
        toggle_body = toggle.group("body")
        mobile_body = mobile.group("body")
        self.assertIn("position: fixed", launcher_body)
        self.assertIn("right: 28px", launcher_body)
        self.assertIn("bottom: 28px", launcher_body)
        self.assertIn("z-index: 190", launcher_body)
        self.assertIn("min-height: 48px", toggle_body)
        self.assertNotIn(".nav-import-btn", self.css)
        self.assertIn(".facai-tools-launcher", mobile_body)
        self.assertIn("display: none", mobile_body)
        self.assertIn(".nav-mobile-utility", mobile_body)
