...
class AiConfigPageTests(unittest.TestCase):
    ...
    def test_ai_config_contains_only_text_model_management(self):
        page = (ROOT / "templates" / "ai_config.html").read_text(encoding="utf-8-sig")

        self.assertIn('id="interfaceList"', page)
        self.assertNotIn('canvas-model-admin', page)
        self.assertNotIn('/api/canvas', page)
        self.assertNotIn('图像生成模型', page)
