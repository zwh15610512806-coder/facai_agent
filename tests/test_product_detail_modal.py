...
class ProductDetailTemplateTests(unittest.TestCase):
    ...
    def test_products_page_has_single_main_rag_chat_that_switches_scope(self):
        ...
        self.assertIn("const payload=scoped?{query:query}:{query:query,limit:5}", self.page)
    ...
    def test_rag_results_render_only_result_rows_with_clickable_sources(self):
        ...
        self.assertIn("function hasRagResults(data)", self.page)
        self.assertIn("hasRagResults(data)?renderRagResults(data)", self.page)
        ...
        self.assertIn("rag-source-chip", body)
        self.assertIn("openSourceViewer", body)
